...
class Ant(Insect):
    ...
    def double(self):
        ...
        if self.has_doubled == False:
            self.damage *= 2
            self.has_doubled = not self.has_doubled
        # END Problem 12
    ...
    def nearest_bee(self):
        ...
        while plc.entrance:
            if distance > self.upper_bound:
                return None
            elif (not plc.is_hive) and plc.bees and distance >= self.lower_bound:
                return random_bee(plc.bees)
            else:
                plc=plc.entrance
                distance+=1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        existing_bees=self.place.bees
        for bee in existing_bees[:]:
...
class WallAnt(Ant):
    """
    WallAnt, an ant that does nothing each turn, but has a large health value.
    """
    name = 'Wall'
    damage = 0
    food_cost = 4
...
class HungryAnt(Ant):
    """
    HungryAnt, which will select a random Bee from its place and deal damage to the Bee by eating it whole.
    """
    name = 'Hungry'
    damage = 0
    food_cost = 4
    ...
    def action(self, gamestate):
        if self.turns_to_chew != 0:
            ...
            if self.place.bees:
                eat_bee=random_bee(self.place.bees)
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while check_place:
            if check_place.ant:
                if check_place.ant.is_container:
                    if check_place.ant.ant_contained:
