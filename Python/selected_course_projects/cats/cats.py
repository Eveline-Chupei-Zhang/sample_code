...
def about(subject):
    ...
    >>> about_dogs = about(['dog', 'dogs', 'pup', 'puppy'])--criteria
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def select(testlongString):
        # pre process of testString
        testlongString = remove_punctuation(testlongString)
        testlongString = lower(testlongString)
        testlongString = split(testlongString)
        for criteria in subject:
            for testsub in testlongString:
                if criteria == testsub:
                    return True
        return False
    ...
    if typed == source:
        ...
        if len(typed_words)<=len(source_words):
            ...
            for i in range(len(source_words)):
                if typed_words[i] == source_words[i]:
                    correct_num += 1
        return 100*correct_num/len(typed_words)
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    cache = {}
    def memoized_diff(typed, source, limit):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for i in range(len(word_list)-1):
        if min > diff_function(typed_word,word_list[i+1],limit) and diff_function(typed_word,word_list[i+1],limit) <= limit:
            min = diff_function(typed_word,word_list[i+1],limit)
            curr = word_list[i+1]
    if min > limit:
        return typed_word
    return curr
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i]==prompt[i]:
            ...
            break
    correct_ratio=matched/len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    for t in times_per_player:
        times.append([t[i+1]-t[i] for i in range(len(t)-1)])
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_word_list=[[] for j in player_indices]
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match, j, i) < min_time:
                ...
                min_player=j
        fastest_word_list[min_player].append(get_word(match, i))
